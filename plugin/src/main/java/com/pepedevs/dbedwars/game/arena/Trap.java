...
public class Trap implements com.pepedevs.dbedwars.api.game.Trap {
                        ...
                        target.getArena()
                                ...
                                .forEach(
                                        tm -> {
                                            if (!tm.equals(team))
                                                tm.getPlayers()
                                                        .forEach(p -> a.forEach(c -> c.accept(p)));
                                        });
    ...
    private void parseTrapAction() {
        cfgTrap.getTrapActions()
                .forEach(
                        a -> {
                            TrapEnum.TargetType targetType =
                                    TrapEnum.TargetType.match(a.getTarget());
                            Set<Consumer<ArenaPlayer>> actions = new HashSet<>();
                            a.getExecutables()
                                    .forEach(
                                            e -> {
                                                for (TrapEnum.ActionType value :
                                                        TrapEnum.ActionType.VALUES) {
