...
public class TeamSizeGui extends IMenu< ItemMenu > {
    ...
    protected void setUpMenu( Player player, @Nullable ItemClickAction action, @Nullable Map< String, Object > info ) {
        ...
        ActionItem main = new ActionItem( StringUtils.translateAlternateColorCodes( "&a&l" + arena.getSettings( ).getTeamPlayers( ) ), XMaterial.ARMOR_STAND.parseItem( ) );
        main.getIcon( ).setAmount( arena.getSettings( ).getTeamPlayers( ) );
        this.menu.setItem( 13, main );

        ActionItem add = new ActionItem( StringUtils.translateAlternateColorCodes( "&eAdd &f&l1" ), XMaterial.LIME_WOOL.parseItem( ),
                StringUtils.translateAlternateColorCodes( "&fClick to add 1" ) );
        add.addAction( new ItemAction( ) {
            @Override
        ...
        ActionItem sub = new ActionItem( StringUtils.translateAlternateColorCodes( "&eRemove &f&l1" ), XMaterial.RED_WOOL.parseItem( ),
                StringUtils.translateAlternateColorCodes( "&fClick to subtract 1" ) );
        sub.addAction( new ItemAction( ) {
            @Override
            public ItemActionPriority getPriority( ) {
                return ItemActionPriority.HIGH;
            }
