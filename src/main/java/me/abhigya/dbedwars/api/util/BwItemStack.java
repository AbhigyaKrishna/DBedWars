...
public class BwItemStack implements Cloneable {
    ...
    public static int getMissing(Player player, Material material, int amount) {
        for (ItemStack itemStack : player.getInventory().getContents()) {
            if (itemStack != null
                    ...
                    && Utils.hasPluginData(itemStack)) amount -= itemStack.getAmount();
        }
        return amount;
    }
    ...
    public ItemStack toItemStack() {
        if (!Utils.hasPluginData(this.item)) this.item = Utils.addPluginData(this.item);
        return this.item;
    }
