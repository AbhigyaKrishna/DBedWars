...
public class PotionEffectAT {
    ...
    public void applyTo(LivingEntity entity) {
        if (DBedwars.getInstance().getServerVersion().isOlderEquals(Version.v1_8_R3)) {
            PotionEffect effect = new PotionEffect(this.potion.parsePotionEffectType(), this.duration, this.amplifier-1);
            effect.apply(entity);
        } else {
            PotionEffect effect = new PotionEffect(this.potion.parsePotionEffectType(), this.duration, this.amplifier-1, true, this.particleOff);
            effect.apply(entity);
        }
    }
    ...
    public String toString() {
        return this.potion.name() + ":" + (Integer.parseInt(String.valueOf(this.amplifier)) - 1) + ":" + (this.duration == MAX_DURATION ? "max" : this.duration)
