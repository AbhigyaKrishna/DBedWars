...
public class IngameTabImpl extends PluginHandler {
    ...
    public IngameTabImpl clearAllPlayers( Player user ) {
        for ( Player player : Bukkit.getOnlinePlayers( ) ) {
            WrappedPacketOutPlayerInfo packet = new WrappedPacketOutPlayerInfo( WrappedPacketOutPlayerInfo.PlayerInfoAction.REMOVE_PLAYER,
                    new WrappedPacketOutPlayerInfo.PlayerInfo( player.getName( ), new WrappedGameProfile( player.getUniqueId( ), player.getName( ) ), GameMode.CREATIVE, 20 ) );
    ...
    public IngameTabImpl showArenaPlayers( Player user ) {
        for ( ArenaPlayer player : this.arena.getPlayers( ) ) {
            WrappedPacketOutPlayerInfo packet = new WrappedPacketOutPlayerInfo( WrappedPacketOutPlayerInfo.PlayerInfoAction.ADD_PLAYER,
                    new WrappedPacketOutPlayerInfo.PlayerInfo( player.getPlayer( ).getName( ), new WrappedGameProfile( player.getPlayer( ).getUniqueId( ), player.getPlayer( ).getName( ) ),
                            player.getPlayer( ).getGameMode( ), PacketEvents.get( ).getPlayerUtils( ).getPing( player.getPlayer( ) ) ) );
            PacketEvents.get( ).getPlayerUtils( ).sendPacket( user, packet );
        }
    ...
    public void handlePlayerJoin( PlayerJoinEvent event ) {
        ...
        if ( !opt.isPresent( ) ) {
            WrappedPacketOutPlayerInfo packet = new WrappedPacketOutPlayerInfo( WrappedPacketOutPlayerInfo.PlayerInfoAction.REMOVE_PLAYER,
                    new WrappedPacketOutPlayerInfo.PlayerInfo( event.getPlayer( ).getName( ), new WrappedGameProfile( event.getPlayer( ).getUniqueId( ), event.getPlayer( ).getName( ) ), GameMode.CREATIVE, 20 ) );
