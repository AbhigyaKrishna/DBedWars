...
public class JSONBuilder implements Cloneable{
    ...
    public JSONBuilder add(String key, Object value, Class<?> clazz){

        if (json.has(key))
            throw new IllegalArgumentException("field" + key + "is already present");

        return adder(key, value, clazz);
    }

    public JSONBuilder addIfNotExists(String key, Object value, Class<?> clazz){
        if (json.has(key))
            return this;
    ...
    @NotNull
    private JSONBuilder adder(String key, Object value, Class<?> clazz) {
        if (clazz.equals(String.class)){
            json.addProperty(key,(String) value);
        }else if (clazz.equals(Number.class)){
            json.addProperty(key,(Number) value);
