...
public class DatabaseUtils {
    ...
    public static void saveGameData( Arena arena ) {
        for ( ArenaPlayer player : arena.getPlayers( ) ) {
            DBedwars.getInstance( ).getThreadHandler( ).addAsyncWork( new Workload( ) {
                @Override
                public void compute( ) {
                    List< DataCache > saved = DBedwars.getInstance( ).getDatabaseBridge( ).getAsDataCache( "UUID", player.getUUIDPlayer( ).getUniqueId( ).toString( ), "PLAYER_STATS" );
                    PlayerStats stat;
                    if ( !saved.isEmpty( ) ) {
                        ...
                        stat = new PlayerStats( player.getUUIDPlayer( ).getUniqueId( ).toString( ), player.getName( ) );
                        stat.setCOINS( 10 );
                        stat.setLEVEL( 1 );
                        stat.setPOINTS( Utils.calculatePoint( player ) );
                    }
                    DBedwars.getInstance( ).getDatabaseBridge( ).saveData( stat, "PLAYER_STATS", !saved.isEmpty( ) );
                }
            } );
        }
    }
