...
public class BedBugDisplayNameUpdateTask implements Workload {
    ...
    private String getHealthLeftString(){
        return (StringUtils.getProgressBar(silverfish.getHealth(),silverfish.getMaxHealth(),cfgBedBug.getHealthIndicatorCount(),cfgBedBug.getHealthSymbol(),
                ChatColor.getByChar(cfgBedBug.getHealthColorCodes().split(":")[0].replaceAll("%team_color%", String.valueOf(teamColor.getChar()))),
                ChatColor.getByChar(cfgBedBug.getHealthColorCodes().split(":")[1].replaceAll("%team_color%", String.valueOf(teamColor.getChar())))));
    }
