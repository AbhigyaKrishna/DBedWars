...
public class HologramRotateTask {
    ...
    public void startRotation(float degreeRotatedPerCycle, int ticksPerAnimationCycle, String onAnimationEnd, boolean slowDownAtEnd, float verticalMovement){
        if(slowDownAtEnd){
            float horizontalAcceleration = (4*degreeRotatedPerCycle)/(ticksPerAnimationCycle*ticksPerAnimationCycle);
            float verticalAcceleration = (4*verticalMovement)/(ticksPerAnimationCycle*ticksPerAnimationCycle);
            plugin.getThreadHandler().addAsyncWork(new Workload() {
                ...
                public void compute() {
                    ...
                    if(tick>=ticksPerAnimationCycle/2){
                        tick++;
                        rotateHologram(revert * (horizontalAcceleration/2) * ((2 * (ticksPerAnimationCycle-tick))+1),(revert * (verticalAcceleration/2) * ((2 * (ticksPerAnimationCycle-tick))+1)));
                    }else {
                        tick++;
                        rotateHologram(revert * (horizontalAcceleration/2) * ((2 * tick)+1),(revert * (verticalAcceleration/2) * ((2 * tick)+1)));
                    }
                }
            ...
            plugin.getThreadHandler().addAsyncWork(new Workload() {
                ...
                public void compute() {
                    tick++;
                    timestamp = System.currentTimeMillis();
                    rotateAndMoveHologram(0,revert * verticalMovement/ticksPerAnimationCycle,0,revert * degreeRotatedPerCycle/ticksPerAnimationCycle,0);
                    if (tick>=ticksPerAnimationCycle){
                        tick = 0;
                        if (onAnimationEnd.equalsIgnoreCase("reverse")){
                            if(revert==1){
    ...
    public void startRotation(float x,float y, float z,float yaw, float pitch,int animationTicks){
        plugin.getThreadHandler().addAsyncWork(new Workload() {
            ...
            public void compute() {
                timestamp = System.currentTimeMillis();
                tick++;
                rotateAndMoveHologram(x/animationTicks,y/animationTicks,z/animationTicks,yaw/animationTicks,pitch/animationTicks);
            }
