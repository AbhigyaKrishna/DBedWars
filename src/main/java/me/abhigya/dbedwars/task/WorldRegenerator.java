    ...
    public Future< World > regenerate( ) {
        ...
        switch ( this.getRegenerationType( ) ) {
            case SINGLE_THREADED:
                world.complete( this.plugin.getGeneratorHandler( ).getWorldAdaptor( ).loadWorldFromSave( this.getArena( ).getSettings( ).getName( ) ) );
                break;
            case MULTI_THREADED_ASYNC:
                this.plugin.getThreadHandler( ).getLeastWorkAsyncWorker( ).add( ( ) ->
                        world.complete( this.plugin.getGeneratorHandler( ).getWorldAdaptor( ).loadWorldFromSave( this.getArena( ).getSettings( ).getName( ) ) ) );
                break;
