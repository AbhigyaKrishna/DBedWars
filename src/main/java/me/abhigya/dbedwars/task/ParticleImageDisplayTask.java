...
public class ParticleImageDisplayTask implements Workload {
    ...
    public ParticleImageDisplayTask(int ticksToDisplayFor, Dimension dimension, Location topLeft, String mainAxisXorZ, Color[][] imagePixels, ParticleBuilder nonLocationParticleBuilder){
        ...
        for (int x = 0; x < imagePixels.length; x++) {
            for (int y = 0; y < imagePixels[0].length; y++) {
                if (imagePixels[x][y] != null){
                    particle.setLocation(topLeft.add(mainAxisMultiplier*x,-yAxisMultiplier*y,0)).setColor(imagePixels[x][y]).display();
