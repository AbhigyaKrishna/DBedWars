...
public class ImageHandler {
    ...
    public void clearCache(){
        for (File file : PluginFiles.PARTICLE_IMAGES_CACHE.getFile().listFiles()) {
            if (!file.delete())
                System.out.println("ERROR DELETING FILE " + file.getName() + " in graphics/particle-images-cache");
        }
    }
    ...
    public void formatAllImagesToPNG(){
        for (File file : PluginFiles.IMAGES.getFile().listFiles()) {
            if (!file.exists())
                continue;
            plugin.getConfigHandler().getParticleImages().getImageSettings().forEach((key, value) -> {
                if (file.getName().equals(key.concat(".").concat(value.getFormat()))) {
                    ImageUtils.downscale(file, dimensionParser(plugin.getConfigHandler().getParticleImages().getGlobalSettings().getDownscaling()), new File(PluginFiles.PARTICLE_IMAGES_CACHE.getFile(), "fuck.png"));
                }
            });
        }
    }
    ...
    private Dimension dimensionParser(String s){
        return new Dimension((int) Double.parseDouble(s.split("x")[0]),(int) Double.parseDouble(s.split("x")[1]));
    }
    ...
    public static class ImageUtils {
        public static void downscale(File image, Dimension dimension, File saveLocation){
            ...
            try {
                ...
                resizedImage = new BufferedImage(dimension.width,dimension.height,BufferedImage.TYPE_INT_ARGB);
        ...
        public static void formatToPNG(File image, File saveLocation){
            ...
            try {
                ...
                downscale(image,new Dimension(image1.getWidth(),image1.getHeight()),saveLocation);
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
                if (image1 != null)
