...
public class ImageHandler {
  ...
  public void clearCache() {
    for (File file : PluginFiles.PARTICLE_IMAGES_CACHE.getFile().listFiles()) {
      if (!file.delete())
        System.out.println(
            "ERROR DELETING FILE " + file.getName() + " in graphics/particle-images-cache");
    }
  }
                  ...
                  ImageUtils.downscale(
                      ...
                      dimensionParser(
                          plugin
                              ...
                              .getGlobalSettings()
                              .getDownscaling()),
                      new File(PluginFiles.PARTICLE_IMAGES_CACHE.getFile(), "fuck.png"));
                }
              });
    }
  }
  ...
  private Dimension dimensionParser(String s) {
    return new Dimension(
        (int) Double.parseDouble(s.split("x")[0]), (int) Double.parseDouble(s.split("x")[1]));
  }
      ...
      try {
        ...
        for (int i = 0; i < width; i++) {
          for (int i1 = 0; i1 < height; i1++) {
            if (!isTransparent(image.getRGB(i, i1))) colors[i][i1] = new Color(image.getRGB(i, i1));
          }
        }
      } catch (IOException e) {
        e.printStackTrace();
