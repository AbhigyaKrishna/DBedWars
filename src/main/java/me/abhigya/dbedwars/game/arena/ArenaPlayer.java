...
import me.abhigya.dbedwars.api.events.PlayerFinalKillEvent;
import me.abhigya.dbedwars.api.events.PlayerKillEvent;
import me.abhigya.dbedwars.api.events.TeamEliminateEvent;
    ...
    if (this.team.isBedBroken()) {
      ...
      if (reason == DeathCause.VOID)
        event
            ...
            .getPlayer()
            .teleport(
                this.arena.getSettings().getSpectatorLocation().toBukkit(this.arena.getWorld()));
      ...
      if (event.getVictim().getTeam().getPlayers().stream()
          .allMatch(me.abhigya.dbedwars.api.game.ArenaPlayer::isFinalKilled)) {
        ...
        if (oTeam.isPresent()
            && this.arena.getTeams().stream().filter(t -> !t.isEliminated()).count() == 1) {
          this.arena.end();
        }
      }
    } else {
