...
import me.abhigya.dbedwars.api.events.SpawnerDropItemEvent;
import me.abhigya.dbedwars.api.events.SpawnerUpgradeEvent;
...
public class Spawner implements me.abhigya.dbedwars.api.game.spawner.Spawner {
                    ...
                    e ->
                        e instanceof Item
                            && ((Item) e)
                                .getItemStack()
                                .getType()
                                .equals(entry.getKey().getItem().getType()))
  ...
  public void spawn(DropType.Drop drop) {
    SpawnerDropItemEvent event =
        new SpawnerDropItemEvent(
            this.arena, this.getDropType(), drop, this, this.team, this.level, this.tier);
    event.call();
    ...
    this.plugin
        ...
        .add(
            () -> {
              ...
              if (this.drop.getSpawnSound() != null) this.drop.getSpawnSound().play(this.location);
  ...
  public boolean upgrade(int level) {
    ...
    SpawnerUpgradeEvent event =
        new SpawnerUpgradeEvent(
            this.arena, this.drop, this, this.team, this.level, level, this.tier, nextTier);
    event.call();
            ...
            () -> {
              ...
              if (effect != null) {
                ParticleBuilder particle =
                    new ParticleBuilder(effect, this.location.clone().add(0, 2, 0));
                particle
                    .setSpeed(0)
                    .setAmount(20)
                    .display(
  ...
  private void broadcast(String key, String message, @Nullable Location location) {
    if (key.equalsIgnoreCase("-all")) {
              ...
              p ->
                  p.sendMessage(
                      ConfigurationUtils.parseMessage(
                          ConfigurationUtils.parsePlaceholder(message, p))));
            ...
            .forEach(
                e ->
                    ((Player) e)
                        .sendMessage(
                            ConfigurationUtils.parseMessage(
                                ConfigurationUtils.parsePlaceholder(message, ((Player) e)))));
      }
      ...
      if (player != null) {
        player.sendMessage(
            ConfigurationUtils.parseMessage(ConfigurationUtils.parsePlaceholder(message, player)));
      }
    }
  }
