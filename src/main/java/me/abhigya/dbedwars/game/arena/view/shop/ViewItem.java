...
public class ViewItem implements me.abhigya.dbedwars.api.game.view.ViewItem {
    ...
    public BwItemStack getFormatted( ) {
        ...
        ItemMetaBuilder builder = stack.getItemMetaBuilder( ).withDisplayName( StringUtils.translateAlternateColorCodes( this.name ) )
    ...
    public ActionItem getActionItem( boolean recreate ) {
        ...
        if ( this.attributes.containsKey( AttributeType.PURCHASABLE ) ) {
            ...
            PlayerPurchaseItemEvent event = new PlayerPurchaseItemEvent( this.player, this.player.getArena( ), cost, items.values( ) );

            if ( this.attributes.containsKey( AttributeType.AUTO_EQUIP ) ) {
                me.abhigya.dbedwars.api.game.view.ViewItem.Attribute equip = this.attributes.get( AttributeType.AUTO_EQUIP );
            ...
            item.addAction( new ItemAction( ) {
                ...
                public void onClick( ItemClickAction itemClickAction ) {
                    ...
                    if ( event.getCost( ).stream( ).allMatch( i -> BwItemStack.playerHas( itemClickAction.getPlayer( ), i ) ) ) {
                        ...
                        itemClickAction.getPlayer( ).sendMessage( StringUtils.translateAlternateColorCodes( "&cYou don't have the required items." ) );
                    }
                }
            } );
        }
        ...
        if ( this.attributes.containsKey( AttributeType.COMMAND ) ) {
            me.abhigya.dbedwars.api.game.view.ViewItem.Attribute command = this.attributes.get( AttributeType.COMMAND );
                ...
                public void onClick( ItemClickAction itemClickAction ) {
                    cmd.forEach( c -> {
                        if ( c.startsWith( "[CONSOLE]" ) ) {
                            Bukkit.getServer( ).dispatchCommand( Bukkit.getConsoleSender( ), c.replace( "[CONSOLE]", "" ).trim( ) );
        ...
        if ( this.attributes.containsKey( AttributeType.CHANGE_PAGE ) ) {
            me.abhigya.dbedwars.api.game.view.ViewItem.Attribute page = this.attributes.get( AttributeType.CHANGE_PAGE );
    ...
    public void override( Overridable override ) throws OverrideException {
        if ( !( override instanceof ViewItem ) )
        ...
        if ( item.getEnchants( ) != null )
            this.enchants = item.getEnchants( );
    ...
    public class Attribute implements me.abhigya.dbedwars.api.game.view.ViewItem.Attribute {
        ...
        protected void load( ConfigurableShop.ConfigurableAttribute atr ) {
            ...
            if ( type == AttributeType.PURCHASABLE ) {
                for ( Map.Entry< String, ConfigurableShop.ConfigurableAttribute.AttributeItems > atrItem : atr.getItemsToGive( ).entrySet( ) ) {
                    if ( atrItem.getValue( ).isInvalid( ) )
                        continue;
                    if ( atrItem.getValue( ).getMaterial( ) != null ) {
                        BwItemStack stack = ConfigurationUtils.parseItem( this.item.player.getTeam( ), atrItem.getValue( ).getMaterial( ) );
                        ...
                        if ( atrItem.getValue( ).getName( ) != null )
                            builder.withDisplayName( StringUtils.translateAlternateColorCodes( atrItem.getValue( ).getName( ) ) );
                        if ( !atrItem.getValue( ).getLore( ).isEmpty( ) )
                            builder.withLore( StringUtils.translateAlternateColorCodes( atrItem.getValue( ).getLore( ) ) );
                        stack.setItemMetaBuilder( builder );
                        if ( atrItem.getValue( ).getEnchant( ) != null )
                            atrItem.getValue( ).getEnchant( ).stream( ).map( LEnchant::valueOf )
                ...
                for ( Map.Entry< String, ConfigurableShop.ConfigurableAttribute.AttributeItems > atrItem : atr.getItemsToGive( ).entrySet( ) ) {
                    this.keyEntry.put( AttributeType.AUTO_EQUIP.getConfigKeys( )[0] + "-" + atrItem.getKey( ), atrItem.getValue( ).getAutoEquipSlot( ) );
                }
            }
        }
        ...
        public boolean isValid( ) {
            ...
            return Arrays.stream( type.getConfigKeys( ) ).allMatch( s -> keys.stream( ).anyMatch( k -> k.contains( s ) ) );
        }
