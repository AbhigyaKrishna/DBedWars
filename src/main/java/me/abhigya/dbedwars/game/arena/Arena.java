...
public class Arena implements me.abhigya.dbedwars.api.game.Arena {
  ...
  public Arena(DBedwars plugin, ConfigurableArena cfg) {
    ...
    this.cfgArena = cfg;
    this.settings =
        new me.abhigya.dbedwars.game.arena.settings.ArenaSettings(this.plugin, this, cfg);
  ...
  public boolean saveData(boolean overwriteData) {
    File file =
        new File(PluginFiles.ARENA_DATA_SETTINGS.getFile(), this.settings.getName() + ".yml");
  ...
  @Override
  public void load() {
    if (!this.enabled) throw new IllegalStateException("Tried loading arena which isn't enabled!");
    ...
    this.plugin
        ...
        .addAsyncWork(
            () -> {
              File file =
                  new File(
                      PluginFiles.ARENA_DATA_SETTINGS.getFile(), this.settings.getName() + ".yml");
  ...
  public boolean end() {
    ...
    ArenaEndEvent event =
        new ArenaEndEvent(
            this,
            this.players.stream().filter(p -> !p.isFinalKilled()).collect(Collectors.toSet()));
    event.call();
  ...
  public boolean kickPlayer(ArenaPlayer player, KickReason reason) {
    if (this.status == ArenaStatus.WAITING) {
      ...
      this.scheduleMessage(
          event.getArena(), event.getPlayer(), event.getArena().getPlayers().size(), false);
      this.players.remove(player);

      if (event.getArena().getPlayers().size() == 0) event.getArena().setStatus(ArenaStatus.IDLING);
    } else {
  ...
  private void scheduleMessage(
      me.abhigya.dbedwars.api.game.Arena arena, Player player, int size, boolean join) {
    this.plugin
        .getThreadHandler()
        .addAsyncWork(
    ...
    this.plugin
        ...
        .addSyncWork(
            new Workload() {
              ...
              public void compute() {
                for (Player player :
                    new ArrayList<>(Arena.this.scoreboard.getHandle().getViewers())) {
