package me.abhigya.dbedwars.configuration.configurablegui;
...
import me.abhigya.dbedwars.api.exceptions.InvalidGuiFileException;
...
public class GuiConfigHandler {
    ...
    public GuiConfigHandler( File file ) throws InvalidGuiFileException {
        ...
        if ( !config.contains( TITLE_KEY ) || !config.contains( PATTERN_KEY ) || !config.contains( ITEM_KEY ) ) {
    ...
    public ItemMenu loadGui( Plugin plugin ) {
        ...
        List< char[] > patternRow = this.parsePattern( config.getStringList( PATTERN_KEY ).stream( ).limit( 6 ).collect( Collectors.toList( ) ) );
        Map< String, ActionItem > actionItems = this.getActionItems( config.getConfigurationSection( ITEM_KEY ) );

        ItemMenu menu = new ItemMenu( StringUtils.translateAlternateColorCodes( title ), ItemMenuSize.fitOf( patternRow.size( ) * 9 ), null );
        int n = 0;
