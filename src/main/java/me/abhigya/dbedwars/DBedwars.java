...
public final class DBedwars extends PluginAdapter {
  ...
  public void onLoad() {
    ...
    if (!PluginFiles.LANGUAGES.getFile().isDirectory()) PluginFiles.LANGUAGES.getFile().mkdirs();

    for (PluginFiles files : PluginFiles.getLanguageFiles()) {
      if (!files.getFile().exists()) {
        this.saveResource(
            "languages/" + files.getFile().getName(), PluginFiles.LANGUAGES.getFile(), false);
      }
    }
    ...
    if (this.getConfigHandler().getDatabase().isValid())
      type =
          ConfigurationUtils.matchEnum(
              this.getConfigHandler().getDatabase().getDatabase(), DatabaseType.values());
  ...
  private boolean checkSWM(org.bukkit.plugin.Plugin plugin) {
    switch (plugin.getDescription().getVersion()) {
      ...
      case "1.0.0-BETA":
        ConsoleUtils.sendPluginMessage(
            ChatColor.DARK_RED
                + "Could not hook into SlimeWorldManager support! You are running an unsupported"
                + " version",
            this.getAlias());
        return false;
    }
