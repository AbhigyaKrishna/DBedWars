  ...
  public void onTNTPlace(BlockPlaceEvent event) {
    ...
    TNTPrimed tntPrimed =
        event
            .getPlayer()
            .getWorld()
            .spawn(event.getBlockPlaced().getLocation().add(0.5, 0, 0.5), TNTPrimed.class);
    tntPrimed.setFuseTicks(cfgTNT.getFuseTicks());
    if (!cfgTNT.isBetterTNTAnimationEnabled()) {
      return;
    }
    tntPrimed.setVelocity(
        VectorUtils.rotateAroundAxisY(new Vector(0.01, 0.40, 0.01), new Random().nextInt(360)));
  ...
  public void onTNTExplode(EntityExplodeEvent event) {
    if (cfgTNT.isFixRandomExplosionEnabled()) {
      event
          .getEntity()
          .teleport(
              event.getEntity().getLocation().clone().getBlock().getLocation().add(0.5, 0, 0.5));
    }
