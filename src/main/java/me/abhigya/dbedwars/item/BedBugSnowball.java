...
public class BedBugSnowball extends PluginActionItem {
    ...
    public static final FixedMetadataValue bedBugBallMeta = new FixedMetadataValue(DBedwars.getInstance(),true);
    ConfigurableCustomItems.ConfigurableBedBug cfgBedBug;

    public BedBugSnowball(DBedwars plugin) {
        super(plugin, StringUtils.translateAlternateColorCodes(DBedwars.getInstance().getConfigHandler().getCustomItems().getBedBug().getItemName()),
                StringUtils.translateAlternateColorCodes((DBedwars.getInstance().getConfigHandler().getCustomItems().getBedBug().getItemLore() == null ? new ArrayList<>() : DBedwars.getInstance().getConfigHandler().getCustomItems().getBedBug().getItemLore())),
    ...
    public void onActionPerform(Player player, EnumAction enumAction, PlayerInteractEvent playerInteractEvent) {
        ...
        if (arena == null ||
                ...
                arena.getAsArenaPlayer(player).get().isSpectator())
            return;
        ...
        bedBugBall.setMetadata("thrower", new FixedMetadataValue(plugin,arena.getAsArenaPlayer(player).get().getTeam().getColor()));
        BwItemStack.removeItem(player,this.toItemStack());
    }

    public void onLand(ProjectileHitEvent event){
        Entity bedBugBall = event.getEntity();
        Silverfish bedBug = (Silverfish) bedBugBall.getWorld().spawnEntity(bedBugBall.getLocation().clone().add(0,1,0), EntityType.SILVERFISH);
        Team throwingTeam = plugin.getGameManager().getArena(bedBugBall.getWorld().getName()).getTeam(
                Color.valueOf(bedBugBall.getMetadata("thrower").get(0).asString())
        );
        plugin.getNMSAdaptor().getBedwarsBedBug(bedBug,throwingTeam).clearDefaultPathfinding().addCustomDefaults().initTargets(1);
        plugin.getThreadHandler().addAsyncWork(new BedBugDisplayNameUpdateTask(bedBug, throwingTeam, cfgBedBug));
    }
}
