...
public class BedBugSnowball extends PluginActionItem {
  ...
  public BedBugSnowball(DBedwars plugin) {
    super(
        ...
        StringUtils.translateAlternateColorCodes(
            DBedwars.getInstance().getConfigHandler().getCustomItems().getBedBug().getItemName()),
        StringUtils.translateAlternateColorCodes(
            (DBedwars.getInstance().getConfigHandler().getCustomItems().getBedBug().getItemLore()
                    == null
                ? new ArrayList<>()
  ...
  public void onActionPerform(
    ...
    bedBugBall.setMetadata(
        "thrower",
        new FixedMetadataValue(plugin, arena.getAsArenaPlayer(player).get().getTeam().getColor()));
  ...
  public void onLand(ProjectileHitEvent event) {
    ...
    Silverfish bedBug =
        (Silverfish)
            bedBugBall
                .getWorld()
                .spawnEntity(bedBugBall.getLocation().clone().add(0, 1, 0), EntityType.SILVERFISH);
    Team throwingTeam =
        plugin
            .getGameManager()
