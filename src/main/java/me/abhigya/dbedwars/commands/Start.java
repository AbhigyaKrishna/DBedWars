...
public class Start implements CommandArgument {
    ...
    public boolean execute(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!this.plugin.getGameManager().containsArena(args[0]) || !this.plugin.getGameManager().getArena(args[0]).isEnabled()) {
            player.sendMessage(StringUtils.translateAlternateColorCodes("&cNo arena found with this name!"));
            return true;
        }
