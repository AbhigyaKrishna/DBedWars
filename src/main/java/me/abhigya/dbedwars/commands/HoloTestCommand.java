...
import me.abhigya.dbedwars.configuration.configurable.ConfigurableHologram;
...
public class HoloTestCommand implements CommandArgument {
    ...
    public boolean execute(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        Hologram hologram3 = DBedwars.getInstance().getHologramFactory().createHologram(UUID.randomUUID().toString(),((Player)commandSender).getLocation(),"OK LOLL");
        hologram3.add(new ItemStack(Material.WOOD),false);
        hologram3.spawn();
        LinkedHashMap<Location,Integer> map = new LinkedHashMap<>();
        Location location = ((Player) commandSender).getLocation().clone().subtract(((Player) commandSender).getLocation());
