package me.abhigya.dbedwars.hooks;
...
public class SlimeWorldManagerHook implements WorldAdaptor {
    ...
    public SlimeWorldManagerHook( DBedwars plugin ) {
        this.plugin = plugin;
        this.slime = (SlimePlugin) this.plugin.getServer( ).getPluginManager( ).getPlugin( "SlimeWorldManager" );
        ...
        SchedulerUtils.runTaskAsynchronously( ( ) -> {
            if ( this.isEnabled( ) && this.config.getBoolean( "configure-slime-sources" ) ) {
                if ( !this.config.getBoolean( "disable-configure-message" ) ) {
                    ConsoleUtils.sendPluginMessage( ChatColor.YELLOW + "We are configuring 'SlimeWorldManager/sources.yml' for our convenience!", this.plugin );
                    ConsoleUtils.sendPluginMessage( ChatColor.YELLOW + "If you have any inconvenience with this feature, please stop the server and disable it in 'DBedWars/hooks/SlimeWorldManager-Hook.yml' " +
                            "and revert your setting from 'SlimeWorldManager/sources.yml.old'. But we highly suggest you to leave this feature on! Or you may not be able to use other DataSources for storage!", this.plugin );
                    ConsoleUtils.sendPluginMessage( ChatColor.YELLOW + "You could disable this message by setting 'disable-configure-message' to 'true' in 'DBedWars/hooks/SlimeWorldManager-Hook.yml'", this.plugin );
                }
    ...
    public boolean saveWorld( String worldName, String name ) {
        try {
            ...
            return true;
        } catch ( WorldAlreadyExistsException | WorldTooBigException | WorldLoadedException |
                InvalidWorldException | IOException e ) {
            e.printStackTrace( );
        }
        return false;
    }
    ...
    public void deleteWorld( String worldName ) {
        if ( Bukkit.getWorld( worldName ) != null )
            this.unloadWorld( worldName, false );
    ...
    private void configureSlimeConfig( ) {
        String sourcesPath = this.config.getString( "config", "plugins/SlimeWorldManager/sources.yml" );
        ...
        if ( !sources.exists( ) ) {
            throw new InvalidPathException( sourcesPath, "Please configure the path for \"SlimeWorldManager/sources.yml\" in \"" + this.file.getPath( ) + "\"" );
        }
        ...
        // File DataSource
        String filePath = this.config.getString( "sources.file.path", "plugins/DBedwars/arena/data/arenacache" );
        ...
        if ( !filePath.equals( oldPath ) ) {
            ...
            if ( slimeFolder.isDirectory( ) ) {
                ...
                for ( File worldFile : slimeWorlds ) {
                    if ( !worldFile.getName( ).endsWith( ".slime" ) )
                        continue;
                    try {
                        FileUtils.copyFile( worldFile, new File( PluginFiles.ARENA_DATA_ARENACACHE.getFile( ), worldFile.getName( ) ) );
                    } catch ( IOException e ) {
                        e.printStackTrace( );
                    }
                }
    ...
    private boolean isDefaultMysql( ConfigurationSection section ) {
        try {
            return !section.getBoolean( "enabled" ) && Objects.equals( section.getString( "host" ), "127.0.0.1" )
                    && section.getInt( "port" ) == 3306 && Objects.equals( section.getString( "username" ), "slimeworldmanager" )
                    && Objects.equals( section.getString( "password" ), "" ) && Objects.equals( section.getString( "database" ), "slimeworldmanager" );
        } catch ( Exception ex ) {
            return false;
        }
    }
    ...
    private boolean isDefaultMongoDB( ConfigurationSection section ) {
        try {
            return !section.getBoolean( "enabled" ) && Objects.equals( section.getString( "host" ), "127.0.0.1" )
                    && section.getInt( "port" ) == 27017 && Objects.equals( section.getString( "auth" ), "admin" )
                    && Objects.equals( section.getString( "username" ), "slimeworldmanager" ) && Objects.equals( section.getString( "password" ), "" )
                    && Objects.equals( section.getString( "database" ), "slimeworldmanager" ) && Objects.equals( section.getString( "collection" ), "worlds" );
        } catch ( Exception ex ) {
            return false;
        }
    }
