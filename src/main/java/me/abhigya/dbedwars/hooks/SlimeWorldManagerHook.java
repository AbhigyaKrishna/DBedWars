...
public class SlimeWorldManagerHook implements WorldAdaptor {
  ...
  public SlimeWorldManagerHook(DBedwars plugin) {
    this.plugin = plugin;
    this.slime =
        (SlimePlugin) this.plugin.getServer().getPluginManager().getPlugin("SlimeWorldManager");
          ...
          if (this.isEnabled() && this.config.getBoolean("configure-slime-sources")) {
            if (!this.config.getBoolean("disable-configure-message")) {
              ConsoleUtils.sendPluginMessage(
                  ChatColor.YELLOW
                      + "We are configuring 'SlimeWorldManager/sources.yml' for our convenience!",
                  this.plugin);
              ConsoleUtils.sendPluginMessage(
                  ChatColor.YELLOW
                      + "If you have any inconvenience with this feature, please stop the server"
                      + " and disable it in 'DBedWars/hooks/SlimeWorldManager-Hook.yml' and revert"
                      + " your setting from 'SlimeWorldManager/sources.yml.old'. But we highly"
                      + " suggest you to leave this feature on! Or you may not be able to use other"
                      + " DataSources for storage!",
                  this.plugin);
              ConsoleUtils.sendPluginMessage(
                  ChatColor.YELLOW
                      + "You could disable this message by setting 'disable-configure-message' to"
                      + " 'true' in 'DBedWars/hooks/SlimeWorldManager-Hook.yml'",
                  this.plugin);
            }
          ...
          l =
              l != null
                  ? (Arrays.stream(availableLoaders)
                          .anyMatch(x -> x.equalsIgnoreCase(config.getString("slime-loader")))
  ...
  public World createWorld(String worldName, World.Environment environment) {
    ...
    spm.setString(
        SlimeProperties.DIFFICULTY, this.getDifficulty(worldName).name().toLowerCase(Locale.ROOT));
    spm.setString(
  ...
  public World loadWorldFromSave(String fileName) {
    ...
    spm.setString(
        SlimeProperties.DIFFICULTY, this.getDifficulty(fileName).name().toLowerCase(Locale.ROOT));
    spm.setBoolean(SlimeProperties.PVP, true);
  ...
  private Difficulty getDifficulty(String worldName) {
    String s =
        this.config.getString(
            worldName + ".difficulty", this.config.getString("default.difficulty", "EASY"));
    try {
  ...
  private World.Environment getEnvironment(String worldName) {
    String s =
        this.config.getString(
            worldName + ".environment", this.config.getString("default.environment", "NORMAL"));
    try {
  ...
  private void configureSlimeConfig() {
    ...
    // File DataSource
    String filePath =
        this.config.getString("sources.file.path", "plugins/DBedwars/arena/data/arenacache");
      ...
      if (slimeFolder.isDirectory()) {
        ...
        for (File worldFile : slimeWorlds) {
          ...
          try {
            FileUtils.copyFile(
                worldFile,
                new File(PluginFiles.ARENA_DATA_ARENACACHE.getFile(), worldFile.getName()));
          } catch (IOException e) {
            e.printStackTrace();
          }
        }
