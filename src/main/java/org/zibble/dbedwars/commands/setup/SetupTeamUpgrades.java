...
public class SetupTeamUpgrades extends CommandNode {

    private final SetupSessionManager manager;
    private final Messaging messaging;

    public SetupTeamUpgrades(SetupSessionManager manager, Messaging messaging) {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) {
            member.sendMessage(PluginLang.SETUP_SESSION_TEAM_UPGRADE_USAGE.asMessage());
            return;
        }
        ...
        setupSession.setupTeamUpgradesNPC(color, player.getLocation());
    }
}
